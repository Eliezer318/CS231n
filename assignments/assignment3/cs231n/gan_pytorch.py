...
def discriminator_loss(logits_real, logits_fake):
    ...
    # loss = bce_loss(logits_real.cpu(), torch.ones(N).cpu().type(dtype))
    # loss += bce_loss(logits_fake.cpu(), torch.zeros(N).cpu().type(dtype))
    loss = (bce_loss(logits_real.cpu(), torch.ones(N).type(dtype)))+(bce_loss(logits_fake.cpu(), torch.zeros(N).type(dtype)))
    pass
...
def generator_loss(logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    loss = bce_loss(logits_fake.cpu(), torch.ones(logits_fake.shape).cpu().type(dtype))
...
def build_dc_classifier(batch_size):
    ...
    return nn.Sequential(
      ...
      nn.Conv2d(1, 32, 5), nn.LeakyReLU(),
      nn.MaxPool2d(2, 2),
      nn.Conv2d(32, 64, 5), nn.LeakyReLU(),
      nn.MaxPool2d(2, 2),
      nn.Flatten(),
      nn.Linear(4 * 4 * 64, 4 * 4 * 64), nn.LeakyReLU(),
      nn.Linear(4 * 4 * 64, 1)
    )
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    return nn.Sequential(
      ...
      nn.Linear(1024, 6272), nn.ReLU(), nn.BatchNorm1d(6272),
      Unflatten(),
