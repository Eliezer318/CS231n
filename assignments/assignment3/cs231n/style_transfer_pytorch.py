...
def gram_matrix(features, normalize=True):
    ...
    norm = (C * H * W) * normalize + (not normalize) * 1
    features = features.view(N, C, H * W)
    return torch.bmm(features, features.permute(0, 2, 1))/norm
...
def tv_loss(img, tv_weight):
    ...
    return content_loss(tv_weight, img[:,:,:,1:], img[:,:,:,:-1]) + content_loss(tv_weight, img[:,:,1:,:], img[:,:,:-1,:])
